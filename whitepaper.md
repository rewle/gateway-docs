...
*Реализовано:* админ-панель — статический frontend, ходящий в REST API того же процесса, что обслуживает Gateway in/out (нет отдельной БД или сервиса управления состоянием — общая БД `gateway`). Доступ к LDAP клиента есть, но только как провайдер логина для самой панели (`src/services/auth`, см. `auth.md`) — ролей партнёров оттуда панель не тянет. Рейт-лимиты на роуты настраиваются в интерфейсе (поле есть в схеме), но не применяются гейтвеем.

### Gateway out

Проксирует трафик наружу с учётом заданных ограничений (allow-листы по IP источника/назначения, инъекция заголовков, field rules). В случае обнаружения на удалённой стороне аналогичного ПО (по `.well-known/gateway`) - устанавливает защищённый канал (подписанные RS256-JWT запросы, см. `gatewayIn.md`).

### Gateway in

Отвечает за модификацию и передачу данных по правилам, заданным в ADM панели. Подробнее в [статье](./gatewayIn.md), включая расхождения с замыслом (нет аггрегации источников и ограничения нагрузки на источники).
